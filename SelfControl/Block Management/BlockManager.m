...
- (BOOL)enterAppendMode {
    ...
    if (![pf enterAppendMode]) return NO;
    hostsBlockingEnabled = YES;
    appendMode = YES;
    return YES;
}
- (BOOL)finishAppending {
    if (!appendMode) return NO;
    ...
    BOOL hostsWritten = [hostBlockerSet writeNewFileContents];
    [pf finishAppending];
    int firewallStatus = [pf refreshPFRules];
    appendMode = NO;
    return hostsWritten && firewallStatus == 0;
}
...
- (BOOL)finalizeBlock {
    ...
    BOOL hostsWritten = YES;
    if (hostsBlockingEnabled) {
        [hostBlockerSet addSelfControlBlockFooter];
        hostsWritten = [hostBlockerSet writeNewFileContents];
    }
    int firewallStatus = [pf startBlock];
    return hostsWritten && firewallStatus == 0;
}
...
- (void)addBlockEntryFromString:(NSString*)entryString {
    NSArray<NSString*>* cleanedEntries = [SCMiscUtilities cleanBlocklistEntry: entryString];
    NSString* cleanedEntry = cleanedEntries.firstObject;
    SCBlockEntry* entry = [SCBlockEntry entryFromString: cleanedEntry ?: entryString];
...
- (NSArray*)commonSubdomainsForHostName:(NSString*)hostName {
    ...
    while ([hostName hasSuffix:@"."]) hostName = [hostName substringToIndex:hostName.length - 1];
    // Expand from the parent of www, never from api.www.example.com.
    NSString* base = [hostName hasPrefix:@"www."] ? [hostName substringFromIndex:4] : hostName;
    BOOL (^belongsTo)(NSString*) = ^BOOL(NSString* domain) {
        return [base isEqualToString:domain] || [base hasSuffix:[@"." stringByAppendingString:domain]];
    };
    if (!isAllowlist && belongsTo(@"instagram.com")) {
        // Instagram's web, mobile and API hosts are separate destinations.
        // Any Instagram entry protects the parent website, including pasted API links.
        base = @"instagram.com";
        [newHosts addObjectsFromArray:@[@"instagram.com", @"www.instagram.com", @"i.instagram.com",
            @"b.i.instagram.com", @"graph.instagram.com", @"api.instagram.com"]];
    }
    ...
    if (belongsTo(@"youtube.com") || belongsTo(@"youtu.be")) {
        [newHosts addObjectsFromArray: @[@"youtube.com", @"www.youtube.com", @"m.youtube.com", @"music.youtube.com", @"youtube-nocookie.com", @"www.youtube-nocookie.com", @"youtu.be"]];
    }

    if (belongsTo(@"netflix.com")) {
        [newHosts addObject: @"assets.nflxext.com"];
        [newHosts addObject: @"codex.nflxext.com"];
        [newHosts addObject: @"nflxext.com"];
    }

    if (base.length && ![base isEqualToString:@"*"]) {
        [newHosts addObject:base];
        [newHosts addObject:[@"www." stringByAppendingString:base]];
        for (NSString* label in @[@"m", @"mobile", @"app", @"api"]) {
            [newHosts addObject:[NSString stringWithFormat:@"%@.%@", label, base]];
        }
    }
...
- (NSArray<SCBlockEntry*>*)relatedBlockEntriesForEntry:(SCBlockEntry*)entry {
    ...
    if(![entry.hostname isValidIPAddress] && (includeCommonSubdomains || !isAllowlist)) {
