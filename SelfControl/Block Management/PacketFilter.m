...
NSString* const kPFAnchorCommand = @"anchor \"org.dayloft\"";
...
- (void)addBlockHeader:(NSMutableString*)configText {
	[configText appendString: @"# Options\n"
	 ...
	 "\n"
	 "#\n"
	 "# org.dayloft ruleset for SelfControl blocks\n"
	 "#\n"];
...
- (BOOL)writeConfiguration {
	...
	return [filterConfiguration writeToFile: @"/etc/pf.anchors/org.dayloft" atomically: true encoding: NSUTF8StringEncoding error: nil];
}
...
- (BOOL)enterAppendMode {
    ...
    appendFileHandle = [NSFileHandle fileHandleForWritingAtPath: @"/etc/pf.anchors/org.dayloft"];
    if (!appendFileHandle) {
...
- (void)appendRulesToCurrentBlockConfiguration:(NSArray<NSDictionary*>*)newEntryDicts {
    ...
    NSFileHandle* fileHandle = [NSFileHandle fileHandleForWritingAtPath: @"/etc/pf.anchors/org.dayloft"];
    if (!fileHandle) {
        NSLog(@"ERROR: Failed to get handle for pf.anchors file while attempting to append rules");
        return;
    }
...
- (int)startBlock {
    if (![self writeConfiguration] || ![self addSelfControlConfig]) return EIO;
...
- (void)writePFToken:(NSString*)token error:(NSError**)error {
	[token writeToFile: @"/etc/DayloftPFToken" atomically: YES encoding: NSUTF8StringEncoding error: error];
}
- (NSString*)readPFToken:(NSError**)error {
	return [NSString stringWithContentsOfFile: @"/etc/DayloftPFToken" encoding: NSUTF8StringEncoding error: error];
}
...
- (int)stopBlock:(BOOL)force {
	...
	for (NSString* line in lines) {
		if ([line rangeOfString: @"org.dayloft"].location == NSNotFound) {
...
- (BOOL)addSelfControlConfig {
	...
	if ([pfConf rangeOfString: @"/etc/pf.anchors/org.dayloft"].location == NSNotFound) {
		[pfConf appendString: @"\n"
		 "anchor \"org.dayloft\"\n"
		 "load anchor \"org.dayloft\" from \"/etc/pf.anchors/org.dayloft\"\n"];
	}

	return [pfConf writeToFile: @"/etc/pf.conf" atomically: true encoding: NSUTF8StringEncoding error: nil];
}

- (BOOL)containsSelfControlBlock {
	NSString* mainConf = [NSString stringWithContentsOfFile: @"/etc/pf.conf" encoding: NSUTF8StringEncoding error: nil];
	return mainConf != nil && [mainConf rangeOfString: @"org.dayloft"].location != NSNotFound;
}
