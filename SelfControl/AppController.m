...
@property (nonatomic, strong) DayloftWindowController* dayloftWindow;
...
- (AppController*) init {
	if(self = [super init]) {
        ...
        if (![defaults_ boolForKey:@"DayloftImportedPreferences"]) {
            NSDictionary* old = [defaults_ persistentDomainForName:@"org.eyebeam.SelfControl"];
            for (NSString* key in @[@"Blocklist", @"BlockAsWhitelist", @"BlockDuration", @"BreaksPerBlock", @"AllowLocalNetworks", @"IncludeLinkedDomains", @"StrictDomainBlocking"]) {
                if (old[key] && ![defaults_ objectForKey:key]) [defaults_ setObject:old[key] forKey:key];
            }
            [defaults_ setBool:YES forKey:@"DayloftImportedPreferences"];
        }
...
- (IBAction)addBlock:(id)sender {
    ...
    if (![sender isKindOfClass:SCDayloftBridge.class] && ![self showBreakBudgetPicker]) {
        return;
    }
...
// The break allowance belongs to the individual block, not a global setting.
// This prompt appears immediately before each block starts and remembers the
// last choice only as a convenience for the next block.
- (BOOL)showBreakBudgetPicker {
    if (![NSThread isMainThread]) {
        __block BOOL result = NO;
        dispatch_sync(dispatch_get_main_queue(), ^{
            result = [self showBreakBudgetPicker];
        });
        return result;
    }

    NSAlert* alert = [NSAlert new];
    alert.messageText = @"Break budget";
    alert.informativeText = @"Choose how many five-minute breaks this block may have. During a break, blocked websites are temporarily available and will lock again automatically.";
    [alert addButtonWithTitle: @"Start Block"];
    [alert addButtonWithTitle: @"Cancel"];

    NSPopUpButton* picker = [[NSPopUpButton alloc] initWithFrame: NSMakeRect(0, 0, 270, 26) pullsDown: NO];
    [picker addItemsWithTitles: @[@"0 breaks — stay locked", @"1 five-minute break", @"2 five-minute breaks", @"3 five-minute breaks"]];
    NSInteger savedBudget = MIN(MAX([defaults_ integerForKey: @"BreaksPerBlock"], 0), 3);
    [picker selectItemAtIndex: savedBudget];
    ...
    [defaults_ setInteger: picker.indexOfSelectedItem forKey: @"BreaksPerBlock"];
    return YES;
}
...
- (NSDictionary*)blockSettingsSnapshot {
    return @{
        ...
        @"IncludeLinkedDomains": [defaults_ valueForKey: @"IncludeLinkedDomains"],
        @"StrictDomainBlocking": [defaults_ valueForKey: @"StrictDomainBlocking"],
        @"BreaksPerBlock": @([defaults_ integerForKey: @"BreaksPerBlock"]),
        @"BlockSoundShouldPlay": [defaults_ valueForKey: @"BlockSoundShouldPlay"],
        @"BlockSound": [defaults_ valueForKey: @"BlockSound"],
        @"EnableErrorReporting": [defaults_ valueForKey: @"EnableErrorReporting"]
    };
}

- (IBAction)scheduleBlock:(id)sender {
    if ([SCUIUtilities blockIsRunning]) {
        NSError* err = [SCErr errorWithCode: 104];
        [SCSentry captureError: err];
        [SCUIUtilities presentError: err];
    ...
    if ([settings_ boolForKey: @"ScheduledBlockPending"]) {
        NSAlert* existingScheduleAlert = [NSAlert new];
        existingScheduleAlert.messageText = @"A strict block is already scheduled";
        NSDate* date = [settings_ valueForKey: @"ScheduledBlockDate"];
        NSDateFormatter* formatter = [NSDateFormatter new];
        formatter.dateStyle = NSDateFormatterMediumStyle;
        formatter.timeStyle = NSDateFormatterShortStyle;
        existingScheduleAlert.informativeText = [NSString stringWithFormat: @"It is set for %@. You can replace it, cancel it, or leave it as-is.", [formatter stringFromDate: date] ?: @"the selected time"];
        [existingScheduleAlert addButtonWithTitle: @"Reschedule"];
        [existingScheduleAlert addButtonWithTitle: @"Cancel Schedule"];
        [existingScheduleAlert addButtonWithTitle: @"Keep Schedule"];
        NSModalResponse response = [existingScheduleAlert runModal];
        if (response == NSAlertSecondButtonReturn) {
            [self cancelScheduledBlock];
            return;
        }
    ...
    NSAlert* alert = [NSAlert new];
    alert.messageText = @"Schedule strict block";
    alert.informativeText = @"SelfControl will save the current blocklist and settings, then start them automatically at this time even if the app is closed. Later edits will not change the saved schedule.";
    [alert addButtonWithTitle: @"Schedule Block"];
    [alert addButtonWithTitle: @"Cancel"];

    NSTextField* startLabel = [NSTextField new];
    startLabel.stringValue = @"Start time";
    startLabel.editable = NO;
    startLabel.selectable = NO;
    startLabel.bezeled = NO;
    startLabel.drawsBackground = NO;
    NSDatePicker* datePicker = [[NSDatePicker alloc] initWithFrame: NSMakeRect(0, 0, 300, 26)];
    datePicker.datePickerStyle = NSDatePickerStyleTextFieldAndStepper;
    datePicker.datePickerElements = NSYearMonthDayDatePickerElementFlag | NSHourMinuteDatePickerElementFlag;
    datePicker.minDate = [NSDate dateWithTimeIntervalSinceNow: 60];
    datePicker.dateValue = [NSDate dateWithTimeIntervalSinceNow: 5 * 60];

    NSTextField* breaksLabel = [NSTextField new];
    breaksLabel.stringValue = @"Breaks allowed";
    breaksLabel.editable = NO;
    breaksLabel.selectable = NO;
    breaksLabel.bezeled = NO;
    breaksLabel.drawsBackground = NO;
    NSPopUpButton* breakPicker = [[NSPopUpButton alloc] initWithFrame: NSMakeRect(0, 0, 300, 26) pullsDown: NO];
    [breakPicker addItemsWithTitles: @[@"0 breaks — stay locked", @"1 five-minute break", @"2 five-minute breaks", @"3 five-minute breaks"]];
    NSInteger savedBudget = MIN(MAX([defaults_ integerForKey: @"BreaksPerBlock"], 0), 3);
    [breakPicker selectItemAtIndex: savedBudget];

    NSStackView* accessoryView = [NSStackView stackViewWithViews: @[startLabel, datePicker, breaksLabel, breakPicker]];
    accessoryView.orientation = NSUserInterfaceLayoutOrientationVertical;
    accessoryView.alignment = NSLayoutAttributeLeading;
    ...
    NSDate* startDate = datePicker.dateValue;
    if ([startDate timeIntervalSinceNow] <= 0) {
        [SCUIUtilities presentError: [SCErr errorWithCode: 315]];
        return;
    }

    [defaults_ setInteger: breakPicker.indexOfSelectedItem forKey: @"BreaksPerBlock"];
    [self installScheduledBlockAtDate: startDate];
}

- (void)installScheduledBlockAtDate:(NSDate*)startDate {
    ...
    [self.xpc ensureDaemonInstalled:^(NSError *error) {
        ...
        NSTimeInterval durationSeconds = MAX([[self->defaults_ valueForKey: @"BlockDuration"] integerValue] * 60, 0);
        [self->settings_ synchronizeSettings];
        [self->defaults_ synchronize];
        NSDictionary* settingsSnapshot = [self blockSettingsSnapshot];
        NSArray<NSString*>* blocklistSnapshot = [[self->defaults_ arrayForKey: @"Blocklist"] copy];
        BOOL isAllowlist = [self->defaults_ boolForKey: @"BlockAsWhitelist"];

        [self.xpc refreshConnectionAndRun: ^{
            [self.xpc scheduleBlockWithControllingUID: getuid()
                                             blocklist: blocklistSnapshot
                                           isAllowlist: isAllowlist
                                             startDate: startDate
                                       durationSeconds: durationSeconds
                                         blockSettings: settingsSnapshot
                                                 reply: ^(NSError *error) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    if (error != nil) {
                        [SCUIUtilities presentError: error];
                        ...
                        confirmation.messageText = @"Strict block scheduled";
                        confirmation.informativeText = [NSString stringWithFormat: @"It will start automatically on %@. It includes %@ five-minute break%@.", [formatter stringFromDate: startDate], [settingsSnapshot[@"BreaksPerBlock"] stringValue], [settingsSnapshot[@"BreaksPerBlock"] integerValue] == 1 ? @"" : @"s"];
                        [confirmation addButtonWithTitle: @"OK"];
                        [confirmation runModal];
                        [SCSentry addBreadcrumb: @"App scheduled a strict block" category: @"app"];
                    }
                    [self->settings_ synchronizeSettingsWithCompletion:^(NSError *syncError) {
                        self.addingBlock = NO;
                        [self refreshUserInterface];
                    }];
                });
...
- (void)cancelScheduledBlock {
    self.addingBlock = YES;
    [self refreshUserInterface];
    [self.xpc cancelScheduledBlockWithReply:^(NSError *error) {
        dispatch_async(dispatch_get_main_queue(), ^{
            if (error != nil) {
                [SCUIUtilities presentError: error];
            } else {
                [SCSentry addBreadcrumb: @"App cancelled a scheduled block" category: @"app"];
            }
            [self->settings_ synchronizeSettingsWithCompletion:^(NSError *syncError) {
                self.addingBlock = NO;
                [self refreshUserInterface];
...
- (void)refreshUserInterface {
	...
	if (self.dayloftWindow) {
        [initialWindow_ orderOut:nil];
        [self.dayloftWindow refresh];
        if (blockIsOn) [defaults_ setBool:YES forKey:@"FirstBlockStarted"];
	...
	if(blockIsOn) { // block is on
		...
		BOOL canStartOrSchedule = [defaults_ integerForKey: @"BlockDuration"] != 0 &&
                                 ([[defaults_ arrayForKey: @"Blocklist"] count] != 0 || [defaults_ boolForKey: @"BlockAsWhitelist"]) &&
                                 !self.addingBlock;
		[submitButton_ setEnabled: canStartOrSchedule];
		[scheduleButton_ setEnabled: canStartOrSchedule || ([settings_ boolForKey: @"ScheduledBlockPending"] && !self.addingBlock)];
		[scheduleButton_ setTitle: [settings_ boolForKey: @"ScheduledBlockPending"] ? @"Manage Schedule…" : @"Schedule…"];
		...
		if(!self.addingBlock) {
			...
			[scheduleButton_ setTitle: @"Saving Schedule…"];
		}

		// if block's off, and we haven't shown it yet, show the first-time modal
		if (![defaults_ boolForKey: @"GetStartedShown"] && NSProcessInfo.processInfo.environment[@"XCTestConfigurationFilePath"] == nil) {
...
- (IBAction)openPreferences:(id)sender {
    if (self.dayloftWindow) { [self.dayloftWindow showPreferences]; return; }
...
- (IBAction)showGetStartedWindow:(id)sender {
    if (self.dayloftWindow) { [self.dayloftWindow showHelp]; return; }
...
- (void)applicationWillFinishLaunching:(NSNotification *)notification {
    ...
    if (NSProcessInfo.processInfo.environment[@"XCTestConfigurationFilePath"] == nil) {
        // Dayloft development builds run in place; packaging handles installation.
    }
}
...
- (void)applicationDidFinishLaunching:(NSNotification *)aNotification {
    ...
    [SCSentry startSentry: @"org.dayloft.Dayloft"];
	...
	[[NSDistributedNotificationCenter defaultCenter] addObserver: self
														selector: @selector(handleConfigurationChangedNotification)
															name: @"DayloftConfigurationChangedNotification"
														  object: nil
    ...
    if (NSProcessInfo.processInfo.environment[@"XCTestConfigurationFilePath"] == nil && ![defaults_ boolForKey:@"isTest"]) {
        self.dayloftWindow = [[DayloftWindowController alloc] initWithBridge:[[SCDayloftBridge alloc] initWithController:self]];
        [initialWindow_ orderOut:nil];
        [self.dayloftWindow showWindow:nil];
    }
    [self refreshUserInterface];
...
- (BOOL)applicationShouldHandleReopen:(NSApplication*)sender hasVisibleWindows:(BOOL)flag {
    if (self.dayloftWindow) [self.dayloftWindow showWindow:nil];
    return YES;
}
...
- (void)takeBreakWithLock:(NSLock*)lockToUse {
    ...
    [settings_ synchronizeSettings];
    [self.xpc takeBreakWithReply:^(NSError * _Nonnull error) {
        dispatch_async(dispatch_get_main_queue(), ^{
            if (error != nil) {
                [SCUIUtilities presentError: error];
            } else {
                [SCSentry addBreadcrumb: @"App started a focus break" category:@"app"];
                [self->settings_ synchronizeSettings];
                [self->timerWindowController_ configurationChanged];
            }
            [lockToUse unlock];
        });
    }];
            ...
            if (error != nil) {
                ...
                [self.xpc refreshConnectionAndRun:^{
                    ...
                    [self.xpc startBlockWithControllingUID: getuid()
                                             ...
                                             blockSettings: [self blockSettingsSnapshot]
...
- (BOOL)openSavedBlockFileAtURL:(NSURL*)fileURL {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName: @"DayloftConfigurationChangedNotification" object: self];
