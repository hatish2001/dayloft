...
- (void)start {
    ...
    // A process can be restarted while it was dispatching a due schedule.
    // Let the normal schedule check retry it instead of leaving it marked as
    // "starting" forever.
    SCSettings* settings = [SCSettings sharedSettings];
    if ([settings boolForKey: @"ScheduledBlockStarting"] && ![SCBlockUtilities anyBlockIsRunning]) {
    ...
    // to start the block
    if ([SCBlockUtilities anyBlockIsRunning] || [SCBlockUtilities blockRulesFoundOnSystem] || [SCDaemonBlockMethods scheduledBlockIsPending] || [SCRecurringSchedule hasEnabledSchedules:[[SCSettings sharedSettings] valueForKey:@"DayloftRecurringSchedules"]]) {
...
- (BOOL)listener:(NSXPCListener *)listener shouldAcceptNewConnection:(NSXPCConnection *)newConnection {
    ...
    if (SecCodeCopySelf(kSecCSDefaultFlags, &ownCode) != errSecSuccess ||
        SecCodeCopySigningInformation(ownCode, kSecCSSigningInformation, &signingInfo) != errSecSuccess) {
        if (ownCode) CFRelease(ownCode);
        CFRelease(guest);
        return NO;
    }
    NSString* team = ((__bridge NSDictionary*)signingInfo)[(__bridge NSString*)kSecCodeInfoTeamIdentifier];
    NSString* requirement = team.length ? [NSString stringWithFormat:
        @"anchor apple generic and (identifier \"org.dayloft.Dayloft\" or identifier \"org.dayloft.cli\") and certificate leaf[subject.OU] = %@", team] : nil;
    OSStatus requirementStatus = requirement ? SecRequirementCreateWithString((__bridge CFStringRef)requirement, kSecCSDefaultFlags, &isSelfControlApp) : errSecCSUnsigned;
    CFRelease(signingInfo); CFRelease(ownCode);
    if (requirementStatus != errSecSuccess) { CFRelease(guest); return NO; }
    ...
    [newConnection.exportedInterface setClasses:[NSSet setWithObjects:NSArray.class, NSDictionary.class, NSString.class, NSNumber.class, NSDate.class, nil] forSelector:@selector(setRecurringSchedules:controllingUID:blockSettings:authorization:reply:) argumentIndex:0 ofReply:NO];
