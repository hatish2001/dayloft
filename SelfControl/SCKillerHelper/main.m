...
#define LOG_FILE @"~/Documents/Dayloft-Repair.log"
...
int main(int argc, char* argv[]) {
	@autoreleasepool {
        ...
        NSMutableString* log = [NSMutableString stringWithString: @"===Dayloft-Repair Log File===\n\n"];
		...
		// print lockfile
		if([fileManager fileExistsAtPath: @"/etc/Dayloft.lock"]) {
			[log appendString: [NSString stringWithFormat: @"Found lock file with contents:\n\n%@\n\n", [NSString stringWithContentsOfFile: @"/etc/Dayloft.lock" encoding: NSUTF8StringEncoding error: NULL]]];
		} else {
		...
		// print org.dayloft pf anchors
		if([fileManager fileExistsAtPath: @"/etc/pf.anchors/org.dayloft"]) {
			[log appendString: [NSString stringWithFormat: @"Found anchor file with contents:\n\n%@\n\n", [NSString stringWithContentsOfFile: @"/etc/pf.anchors/org.dayloft" encoding: NSUTF8StringEncoding error: nil]]];
		}
		...
		task = [NSTask launchedTaskWithLaunchPath: @"/bin/launchctl"
										arguments: @[@"unload",
                                                     @"-w",
													 @"/Library/LaunchDaemons/org.dayloft.Dayloft.plist"]];
		[task waitUntilExit];
        status = [task terminationStatus];
        ...
        SMJobRemove(kSMDomainSystemLaunchd, CFSTR("org.dayloft.selfcontrold"), NULL, YES, &cfError);
                                     );
        if (cfError) {
		...
		// remove PF token
		if([fileManager removeItemAtPath: @"/etc/DayloftPFToken" error: nil]) {
		...
		if([fileManager fileExistsAtPath: @"/etc/pf.anchors/org.dayloft"]) {
			if([fileManager removeItemAtPath: @"/etc/pf.anchors/org.dayloft" error: nil])
		...
		// remove lockfile
		if([fileManager fileExistsAtPath: @"/etc/Dayloft.lock"]) {
			if([fileManager removeItemAtPath: @"/etc/Dayloft.lock" error: nil])
