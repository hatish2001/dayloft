...
- (void) testCleanBlocklistEntries {
    ...
    XCTAssert(cleaned.count == 1 && [[cleaned firstObject] isEqualToString: @"cnn.com"]);
    ...
    // A pasted post URL blocks its whole host, including direct typed URLs.
    cleaned = [SCMiscUtilities cleanBlocklistEntry: @"reddit.com/r/selfcontrol/comments/abc123/a-post?sort=new#comment"];
    XCTAssert(cleaned.count == 1 && [[cleaned firstObject] isEqualToString: @"reddit.com"]);

    // A pasted Google result URL unwraps the result and blocks the destination,
    // rather than just blocking Google itself.
    cleaned = [SCMiscUtilities cleanBlocklistEntry: @"https://www.google.com/url?sa=t&url=https%3A%2F%2Fwww.reddit.com%2Fr%2Fselfcontrol%2Fcomments%2Fabc123"];
    XCTAssert(cleaned.count == 1 && [[cleaned firstObject] isEqualToString: @"www.reddit.com"]);
    ...
    XCTAssert([cleaned[0] isEqualToString: @"cnn.com"]);
...
- (void) testModernBlockDetection {
    ...
    [settings setValue: [NSDate dateWithTimeIntervalSinceNow: 300] forKey: @"BreakEndDate"];
    XCTAssert([SCBlockUtilities currentBreakIsActive]);

    [settings setValue: [NSDate dateWithTimeIntervalSinceNow: -1] forKey: @"BreakEndDate"];
    XCTAssert(![SCBlockUtilities currentBreakIsActive]);

    [settings setValue: @YES forKey: @"ScheduledBlockPending"];
    XCTAssert([SCBlockUtilities scheduledBlockIsPending]);
