...
- (void)updateTimerDisplay:(NSTimer*)timer {
    ...
    if ([SCBlockUtilities modernBlockIsRunning]) {
        addToBlockButton_.enabled = ![settings_ boolForKey: @"ActiveBlockAsWhitelist"] && ![SCBlockUtilities currentBreakIsActive];
    }

    [self updateBreakButton];
}

- (void)updateBreakButton {
    if (![SCBlockUtilities modernBlockIsRunning]) {
        breakButton_.hidden = YES;
        return;
    }

    NSInteger maxBreaks = MIN(MAX([[settings_ valueForKey: @"MaxBreaksPerBlock"] integerValue], 0), 3);
    NSInteger usedBreaks = MAX([[settings_ valueForKey: @"BreaksUsed"] integerValue], 0);
    if ([SCBlockUtilities currentBreakIsActive]) {
        NSInteger secondsLeft = MAX((NSInteger)[[settings_ valueForKey: @"BreakEndDate"] timeIntervalSinceNow], 0);
        breakButton_.hidden = NO;
        breakButton_.enabled = NO;
        breakButton_.title = [NSString stringWithFormat: @"Break: %ld:%02ld left", (long)(secondsLeft / 60), (long)(secondsLeft % 60)];
        return;
    }

    NSInteger remainingBreaks = maxBreaks - usedBreaks;
    ...
    breakButton_.hidden = NO;
    breakButton_.enabled = YES;
    breakButton_.title = [NSString stringWithFormat: @"Take 5-min break (%ld left)", (long)remainingBreaks];
}
...
- (IBAction)takeBreak:(id)sender {
    ...
    NSInteger usedBreaks = MAX([[settings_ valueForKey: @"BreaksUsed"] integerValue], 0);
    NSInteger remainingBreaks = maxBreaks - usedBreaks;
    if (remainingBreaks <= 0 || [SCBlockUtilities currentBreakIsActive]) {
        [modifyBlockLock unlock];
        return;
    }

    NSAlert* alert = [NSAlert new];
    alert.messageText = @"Start a five-minute break?";
    alert.informativeText = [NSString stringWithFormat: @"This uses one of your %ld remaining breaks. The website block will return automatically when the break ends.", (long)remainingBreaks];
    [alert addButtonWithTitle: @"Start Break"];
    [alert addButtonWithTitle: @"Cancel"];
    [alert beginSheetModalForWindow: self.window completionHandler:^(NSModalResponse response) {
        if (response == NSAlertFirstButtonReturn) {
            [self.appController takeBreakWithLock: self->modifyBlockLock];
        } else {
            [self->modifyBlockLock unlock];
        }
    }];
}
...
- (IBAction)killBlock:(id)sender {
    ...
    //        SentryAttachment* fileAttachment = [[SentryAttachment alloc] initWithPath: [@"~/Documents/Dayloft-Repair.log" stringByExpandingTildeInPath]];
    ...
    if ([SCBlockUtilities anyBlockIsRunning]) {
        ...
        [alert setInformativeText:@"The block was cleared successfully.  You can find the log file, named Dayloft-Repair.log, in your Documents folder. If you're still having issues, please check out the SelfControl FAQ on GitHub."];
