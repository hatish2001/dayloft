...
+ (NSArray<NSString*>*) cleanBlocklistEntry:(NSString*)rawEntry {
    ...
    // A block always applies to a host, never to one path.  Parse web URLs first so
    // pasted links work the same way as a typed address.  This also unwraps a Google
    // result URL when somebody pastes one directly from a search result.
    BOOL looksLikeURL = [str rangeOfString: @"://"].location != NSNotFound ||
                        [str rangeOfString: @"/"].location != NSNotFound ||
                        [str rangeOfString: @"?"].location != NSNotFound ||
                        [str rangeOfString: @"#"].location != NSNotFound;
    BOOL isIPv4CIDR = [str rangeOfString: @"^([0-9]{1,3}\\.){3}[0-9]{1,3}/[0-9]{1,3}$"
                                      options: NSRegularExpressionSearch].location != NSNotFound;
    if (looksLikeURL && !isIPv4CIDR) {
        NSString* urlString = [str rangeOfString: @"://"].location == NSNotFound
            ? [@"https://" stringByAppendingString: str]
            : str;
        NSURLComponents* components = [NSURLComponents componentsWithString: urlString];
        NSString* host = components.host.lowercaseString;

        if (host.length > 0) {
            BOOL isGoogleRedirect = [host hasSuffix: @"google.com"] ||
                                    [host rangeOfString: @".google."].location != NSNotFound;
            if (isGoogleRedirect && [components.path isEqualToString: @"/url"]) {
                for (NSURLQueryItem* item in components.queryItems) {
                    if (([item.name isEqualToString: @"q"] || [item.name isEqualToString: @"url"]) && item.value.length > 0) {
                        NSArray<NSString*>* targetEntries = [SCMiscUtilities cleanBlocklistEntry: item.value];
                        if (targetEntries.count > 0) return targetEntries;
                    }
                }
            }
            return @[host];
...
+ (NSDictionary*) defaultsDictForUser:(uid_t) controllingUID {
    ...
    [defaults addSuiteNamed: @"org.dayloft.Dayloft"];
